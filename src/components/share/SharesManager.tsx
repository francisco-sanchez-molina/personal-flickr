...
export interface GalleryShareRow {
  token: string;
  gallery_id: number;
  created_at: number;
  view_count: number;
...
function publicUrl(prefix: string, token: string): string {
  if (typeof window === "undefined") return `${prefix}${token}`;
  return `${window.location.origin}${prefix}${token}`;
}
...
export default function SharesManager({
  ...
  initial: ShareRow[];
  galleryInitial?: GalleryShareRow[];
}) {
  const [rows, setRows] = useState<ShareRow[]>(initial);
  const [galleryRows, setGalleryRows] = useState<GalleryShareRow[]>(galleryInitial);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState<string | null>(null);
  const confirm = useConfirm();

  const totalViews =
    rows.reduce((n, r) => n + r.view_count, 0) +
    galleryRows.reduce((n, r) => n + r.view_count, 0);
  const totalLinks = rows.length + galleryRows.length;

  const copy = async (prefix: string, token: string) => {
    try {
      await copyToClipboard(publicUrl(prefix, token));
      setCopied(token);
  ...
  const revokePhoto = async (row: ShareRow) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/shares/${row.token}`, { method: "DELETE" });
  ...
  const revokeGallery = async (row: GalleryShareRow) => {
    const ok = await confirm({
      title: "¿Revocar este enlace?",
      description: `La galería "${row.gallery_name}" dejará de ser accesible sin login.`,
      confirmLabel: "Revocar",
      destructive: true,
    });
    if (!ok) return;
    setError(null);
    try {
      const res = await fetch(`/api/gallery-shares/${row.token}`, { method: "DELETE" });
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error ?? "error");
      setGalleryRows((arr) => arr.filter((s) => s.token !== row.token));
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    }
  };
  ...
  if (totalLinks === 0) {
    return (
      <EmptyState
        ...
        sub='Abre una foto (o una galería) y pulsa "Compartir" para crear el primero.'
      />
    );
  }

  const meta = (row: { created_at: number; view_count: number; last_viewed_at: number | null }) => (
  ...
  return (
    <>
      <div className="mb-3 font-mono text-[11px] tracking-[.04em] text-ink-3 uppercase">
        {totalLinks} enlace{totalLinks === 1 ? "" : "s"} · {totalViews} vista
        {totalViews === 1 ? "" : "s"} en total
      </div>

      <div className="grid gap-2">
        {/* Gallery shares first (broader scope), then individual photos. */}
        {galleryRows.map((row) => (
          <div
            key={row.token}
          ...
          >
            <a
              ...
              className="grid h-14 w-14 shrink-0 place-items-center rounded border border-line bg-bg-3 text-ink-3"
              aria-label={`Abrir ${row.gallery_name}`}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 overflow-hidden font-mono text-[11.5px] tracking-[.02em] text-ellipsis whitespace-nowrap text-ink-3">
                {publicUrl("/sg/", row.token)}
              </div>
              {meta(row)}
            </div>
            <IconButton
              onClick={() => copy("/sg/", row.token)}
              aria-label="Copiar URL"
              title={copied === row.token ? "Copiado ✓" : "Copiar"}
            >
              {copied === row.token ? <Icons.Check size={14} /> : <Icons.Copy size={14} />}
            </IconButton>
            <IconButton
              onClick={() => revokeGallery(row)}
        ...
        {rows.map((row) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 overflow-hidden font-mono text-[11.5px] tracking-[.02em] text-ellipsis whitespace-nowrap text-ink-3">
                {publicUrl("/s/", row.token)}
              </div>
              {meta(row)}
            </div>

            <IconButton
              onClick={() => copy("/s/", row.token)}
              aria-label="Copiar URL"
              title={copied === row.token ? "Copiado ✓" : "Copiar"}
            >
              {copied === row.token ? (
